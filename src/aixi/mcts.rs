...
pub trait AgentSimulator {
    ...
    fn get_explore_exploit_ratio(&self) -> f64 { 1.0 }
...
impl SearchNode {
    ...
    fn select_action(&mut self, agent: &mut dyn AgentSimulator, horizon: usize) -> (&mut SearchNode, Action) {
    ...
    pub fn sample(&mut self, agent: &mut dyn AgentSimulator, horizon: usize, total_horizon: usize) -> f64 {
        if horizon == 0 {
        ...
        if self.is_chance_node {
            ...
            let child = self.children.entry(key).or_insert_with(|| SearchNode::new(false));
...
impl SearchTree {
    ...
    pub fn search(&mut self, agent: &mut dyn AgentSimulator, prev_obs: u64, prev_rew: u64, prev_act: u64, samples: usize) -> Action {
    ...
    fn prune_tree(&mut self, agent: &mut dyn AgentSimulator, prev_obs: u64, prev_rew: u64, prev_act: u64) {
