...
use rwkvzip::{Compressor, State, Model};
use std::sync::Arc;
...
pub trait Predictor {
    ...
    fn predict_one(&mut self) -> f64 { self.predict_prob(true) }
...
impl CtwPredictor {
    ...
    pub fn new(depth: usize) -> Self {
        Self { tree: ContextTree::new(depth) }
    }
}
...
impl RwkvPredictor {
    ...
    pub fn new(model: Arc<Model>) -> Self {
        ...
        let logits = compressor.model.forward(&mut compressor.scratch, 0, &mut compressor.state);
...
impl Predictor for RwkvPredictor {
    fn update(&mut self, sym: bool) {
        ...
        self.history.push((self.compressor.state.clone(), self.compressor.pdf_buffer.clone()));
