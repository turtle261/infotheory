...
impl CtNode {
    ...
    pub fn update(&mut self, sym: Symbol, revert: bool) {
        ...
        let log_prob_w0 = self.children[0].as_ref().map(|c| c.log_prob_weighted).unwrap_or(0.0);
        let log_prob_w1 = self.children[1].as_ref().map(|c| c.log_prob_weighted).unwrap_or(0.0);
    ...
    fn sanity_check(&mut self) {
        if self.log_prob_kt > 1.0e-10 { self.log_prob_kt = 0.0; }
        if self.log_prob_weighted > 1.0e-10 { self.log_prob_weighted = 0.0; }
    }
}
...
impl ContextTree {
    ...
    pub fn update(&mut self, sym: Symbol) {
     ...
     fn update_node(node: &mut CtNode, context: &[Symbol], sym: Symbol, revert: bool, depth: usize) -> bool {
