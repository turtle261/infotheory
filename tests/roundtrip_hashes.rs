...
fn zpaq_roundtrip_fixture_a_and_hash_stability() {
    ...
    let compressed = compress_bytes_backend(&input, &backend).expect("compress failed");
    let restored = decompress_bytes_backend(&compressed, &backend).expect("decompress failed");
