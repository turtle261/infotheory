...
use infotheory::aixi::model::{Predictor, CtwPredictor, RosaPredictor};
use infotheory::aixi::agent::{Agent, AgentConfig};
use infotheory::aixi::environment::{Environment, CtwTest, CoinFlip};
...
fn test_predictor_sum_to_one(mut predictor: Box<dyn Predictor>, name: &str) {
    ...
    // Check range
    assert!(p_true >= 0.0 && p_true <= 1.0, "{name}: Prob out of range: {p_true}");
}
...
fn test_predictor_revert(mut predictor: Box<dyn Predictor>, name: &str) {
    ...
    assert!((prob_reverted - 0.5).abs() < 0.1, 
    ...
    assert!((prob_redo - prob_after_updates).abs() < 1e-9,
...
fn ctw_test_env_is_deterministic() {
    ...
    for i in 0..50 {
        ...
        assert_eq!(env1.get_observation(), env2.get_observation(), "Obs mismatch at step {i}");
        assert_eq!(env1.get_reward(), env2.get_reward(), "Reward mismatch at step {i}");
    }
}
...
fn agent_solves_ctw_test_environment() {
    ...
    println!("Agent Total Reward on CtwTest (100 cycles): {}", total_reward);
    
    // Agent should learn pattern and get reasonable reward
    assert!(total_reward > 50.0, "Agent failed to learn CtwTest pattern. Reward: {total_reward}");
}
...
fn agent_regret_sublinear_coinflip() {
    ...
    println!("CoinFlip(0.8): Reward={total_reward}, Opt={expected_optimal}, Regret/step={regret_per_step:.4}");
