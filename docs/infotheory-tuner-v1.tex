...
Let:
\begin{itemize}
    \item $d$ denote the canonical dataset object loaded from
    \texttt{input\_path},
    \item $x \in \{0,\dots,255\}^{n}$ denote the passive-byte specialization
    of $d$ when \texttt{dataset\_kind} selects the passive-bytes mode,
...
\begin{itemize}
    ...
    \item \texttt{dataset\_kind: DatasetKind}
    \item \texttt{objective\_target: ObjectiveTarget}
...
Define dataset mode by
\[
\texttt{DatasetKind}\in
\left\{
\begin{aligned}
...
Define evaluator target semantics by
\[
\texttt{ObjectiveTarget}\in
\left\{
\begin{aligned}
&\texttt{"passive\_ac"},\\
...
\begin{itemize}
    \item if \texttt{dataset\_kind} selects the passive-bytes mode, then
    \texttt{input\_path} loads a raw byte string, and the normative default
    \texttt{objective\_target} is \texttt{"passive\_ac"},
    \item if \texttt{dataset\_kind} selects the interactive-trace mode, then
    \texttt{input\_path} loads a canonical typed causal trace, and
    \texttt{objective\_target} must select either the interactive-causal-AC
    mode or the planner-deployable-model mode,
    \item if \texttt{dataset\_kind} selects the causal-prefix-dataset mode, then
    \texttt{input\_path} loads a canonical finite causal prediction dataset
    (equivalently, a prefix tree/DAG of prediction examples), and
    \texttt{objective\_target} must select either the interactive-causal-AC
    mode or the planner-deployable-model mode,
    \item if \texttt{objective\_target} selects the
    planner-deployable-model mode, it
    preserves the same causal code-length objective as
    \texttt{"interactive\_causal\_ac"}; it changes only deployability
    constraints and diagnostics as specified in
    Section~\ref{sec:causal-evaluator}.
\end{itemize}
...
Define planner reward mode by
\[
\texttt{PlannerRewardMode}\in
\left\{
\begin{aligned}
&\texttt{"exact\_objective\_difference"},\\
&\texttt{"normalized\_clipped\_improvement"},\\
&\texttt{"quantized\_objective\_difference"}
\end{aligned}
\right\}.
\]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{planner\_reward\_mode: PlannerRewardMode}
        \item \texttt{planner\_simulations\_per\_step}\((\in\mathbb{N},\ge 1)\)
    \end{itemize}
    \item for all planner reward modes additionally:
    \begin{itemize}
        \item \texttt{reward\_bits}
    \end{itemize}
    \item for reward mode
    \texttt{exact\_objective\_difference} optionally, when using the
    legacy affine integer exact-encoding shortcut:
    \begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{planner\_reward\_mode} must be
        \texttt{"normalized\_clipped\_}\allowbreak\texttt{improvement"}
        ...
        \item \texttt{reward\_bits} defines the finite immediate-reward symbol channel
        used to encode normalized clipped one-step improvement rewards.
        \item \texttt{min\_reward} and \texttt{max\_reward} define the
        clipping interval, with \texttt{max\_reward > min\_reward}
    \end{itemize}
    \item for \texttt{aiqi\_warmstart\_exact\allowbreak\_jh} additionally:
    \begin{itemize}
        \item \texttt{planner\_reward\_mode} must be one of the two
        objective-difference modes defined above
        \item \texttt{return\_horizon}\(\in\mathbb{N},\ge 1\)
    ...
    \end{itemize}
    \item for \texttt{mc\_aixi\_fac\_ctw} additionally,
    \texttt{planner\_reward\_mode} must be the exact
    objective-difference mode defined above.
    \item for
    \texttt{objective\_target} selects the planner-deployable-model mode
    additionally:
    \begin{itemize}
        \item \texttt{min\_predicts\_per\_second}\((\in\mathbb{R}_{>0})\)
        \item \texttt{max\_model\_state\_bytes}\((\in\mathbb{N},>0)\)
        \item \texttt{max\_clone\_nanoseconds}\((\in\mathbb{N},>0)\)
        \item \texttt{max\_snapshot\_bytes}\((\in\mathbb{N},>0)\)
        \item \texttt{max\_update\_nanoseconds}\((\in\mathbb{N},>0)\)
        \item \texttt{max\_restore\_nanoseconds}\((\in\mathbb{N},>0)\)
        \item \texttt{required\_sampling: bool}
        \item \texttt{required\_log\_prob: bool}
    \end{itemize}
...
searched object: every candidate still denotes a predictor. What changes is the
dataset lowering and, when requested, the deployability profile.

\subsection{Unified chronological predictor object}
...
A candidate $z\in\mathcal{Z}$ denotes a chronological conditional predictor
\[
...
$z\in\mathcal{Z}$ denote a chronological conditional predictor $\rho_z$.
...
percept term.
Then for every candidate predictor $\rho_z$,
\[
\mathbb{E}_{\pi,\nu}
\left[
\sum_{t=0}^{T}
...
The evaluator accepts exactly one canonical dataset object, selected by
\texttt{dataset\_kind}, and lowers it to a typed causal event stream.

\paragraph{Passive bytes.}
If \texttt{dataset\_kind} selects the passive-bytes mode, then
\texttt{input\_path} loads a raw byte string
...
If \texttt{dataset\_kind} selects the interactive-trace mode, then
\texttt{input\_path} loads a canonical typed causal trace. Its file header must
fix at least:
\begin{itemize}
    \item schema version,
...
If \texttt{dataset\_kind} selects the causal-prefix mode, then
\texttt{input\_path} loads a canonical finite causal prediction dataset.
Equivalently, it loads a prefix tree/DAG of histories with weighted
...
If \texttt{objective\_target} selects the planner-deployable-model mode, the
primary score
remains the same predictor-aligned two-part objective
\[
8L_B(z)+\ell_D(z).
\]
...
loaded from \texttt{input\_path}. In passive-byte mode, $d=x$. All cache
...
\begin{itemize}
    \item \texttt{dataset\_kind} and \texttt{objective\_target},
    ...
    \item build/features, evaluator determinism settings, and warmup policy.
\end{itemize}
...
byte string under the candidate predictor.
...
This section specifies the normative planner-reward modes.
...
\subsection{Planner reward modes and encoded reward semantics}

Planner controllers expose one of the following reward modes:
\begin{itemize}
...
\]
according to the selected reward mode.

For \texttt{exact\_objective\_difference}, the implementation must fix a
...
The interval fields \texttt{min\_reward}, \texttt{max\_reward}, and
\texttt{reward\_offset} are optional legacy conveniences for the special case
where $\mathbb{V}_H^{\mathcal J}$ is an integer interval encoded by an affine
integer map; they are not the general exact reward semantics.

For \texttt{quantized\_objective\_difference}, the implementation must declare
...
For \texttt{normalized\_clipped\_improvement}, define
\[
...
\]
This mode is the normative reward mode for
...
For all planner reward modes, define
\[
...
For \texttt{planner\_reward\_mode = "exact\_objective\_difference"},
...
For \texttt{planner\_reward\_mode = "quantized\_objective\_difference"}, the
...
For \texttt{planner\_reward\_mode = "normalized\_clipped\_improvement"}, the
runtime verifies only the normalized-clipping preconditions required by
Section~\ref{sec:aiqi-discounted-contract}.
\end{remark}

For any reward mode with finite encoded reward alphabet, the emitted percept is
\[
x_{t+1}^{\mathrm{enc}}=
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item if \texttt{planner\_reward\_mode} is
        \texttt{"exact\_objective\_difference"}, verify the exact
        ...
        \item if \texttt{planner\_reward\_mode} is
        \texttt{"quantized\_objective\_difference"}, verify that the declared
        ...
        \item if \texttt{planner\_reward\_mode} is
        \texttt{"normalized\_clipped\_}\allowbreak\texttt{improvement"}, verify
...
It requires
\texttt{planner\_reward\_mode = "normalized\_clipped\_improvement"}.
...
\texttt{planner\_reward\_mode = "normalized\_clipped\_improvement"}, the
...
\texttt{planner\_reward\_mode = "quantized\_objective\_difference"}, but
...
$z\mapsto\rho_z$ across ordinary compression and environment-model tuning, and
it keeps planner-oriented deployment constraints in $H$ without replacing the
primary predictor-aligned codelength objective by return.
